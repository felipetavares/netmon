...
void TextBox::draw(NVGcontext* ctx) {
    ...
    if (mCommitted) {
        ...
        mIsPassword?hideValue(mValue, '*'):mValue;

        // Change A
        nvgText(ctx, drawPos.x(), drawPos.y(), finalValue.c_str(), nullptr);
    } else {
        std::string finalValue =
        mIsPassword?hideValue(mValueTemp, '*'):mValueTemp;
        ...
        // Change B
        nvgTextBounds(ctx, drawPos.x(), drawPos.y(), finalValue.c_str(),
                      nullptr, textBound);
        ...
        int nglyphs =
            nvgTextGlyphPositions(ctx, drawPos.x(), drawPos.y(),
                                  finalValue.c_str(), nullptr, glyphs, maxGlyphs);
...
std::string TextBox::hideValue(std::string &v, char c) {
